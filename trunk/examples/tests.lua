...
function testJSON4Lua()
  ...
  -- Tests are deactivated since these methods are now pricate
  if nil then
  s = "   \n   \r   \t   "
  e = json.decode_scanWhitespace(s,1)
  assert(e==string.len(s)+1)
  s = " \n\r\t4"
  assert(json.decode_scanWhitespace(s,1)==5)
  
  -- Test decode_scanString
  s = [["Test"]]
  r,e = json.decode_scanString(s,1)
  assert(r=='Test' and e==7)
  s = [["This\nis a \"test"]]
  r = json.decode_scanString(s,1)
  assert(r=="This\nis a \"test")
  
  -- Test decode_scanNumber
  s = [[354]]
  r,e = json.decode_scanNumber(s,1)
  assert(r==354 and e==4)
  s = [[ 4565.23 AND OTHER THINGS ]]
  r,e = json.decode_scanNumber(s,2)
  ...
  s = "true"
  r,e = json.decode_scanConstant(s,1)
  assert(r==true and e==5)
  s = "  false  "
  r,e = json.decode_scanConstant(s,3)
